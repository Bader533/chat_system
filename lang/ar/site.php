<?php

return [
    'dashboard' => 'الرئيسية',
    'home' => 'الرئيسية',
    'Logout' => 'تسجيل الخروج',
    'employee' => 'الموظفين',
    'index' => 'كافة الموظفين',
    'create' => 'اضافة ',
    'create_employess' => 'اضافة موظف جديد',
    // 'name' => 'الاسم الاول',
    'name_en' => 'الاسم الانجليزى',
    'name_ar' => 'الاسم العربى',
    'mother_name' => 'الاسم الام',
    'lastn' => 'الاسم الاخير',
    'gender' => 'الجنس',
    'dofb' => 'تاريخ الميلاد',
    'phone' => 'رقم الهاتف',
    'other_phone' => 'رقم هاتف اخر',
    'email' => 'الايميل',
    'career' => 'المهنة',
    'place_of_work' => 'مكان العمل',

    'identity' => 'الهوية',
    'date_of_issuing_the_id' => 'تاريخ اصدار الهوية',
    'place_issue_of_id' => 'مكان اصدار الهوية',

    'date_of_birth' => 'تاريخ الميلاد',
    'place_of_birth' => 'مكان الولادة',

    'account_statement' => 'كشف حساب',
    'credit_inquiry' => 'استعلام اتمانى',

    // 'password' => 'كلمة المرور',
    // 'cpassword' => 'تاكيد كلمة المرور',
    'male' => 'ذكر',
    'female' => 'انثى',
    'all_employee' => 'كل الموظفين',
    'sitting' => 'الاعدادات',
    'language' => 'اللغة',
    'account' => 'الحساب الشخصى',
    'zones' => 'المناطق',
    'export-shippment' => 'استخراج الشحنات',
    'export-drivers' => 'استخراج السائقين',
    'save_changes' => 'حذف التعديلات',
    'id' => 'الرقم',
    'city' => 'المدينة',
    'rate' => 'السعر',
    'add_city' => 'اضافة مدينة',
    'add_new_city' => 'اضف مدينة جديدة',
    'add' => 'أضف ',
    'close' => 'الغاء',
    'area' => 'منطقة',
    'delete' => 'حذف',
    'all_city' => 'كل المدن',
    'update' => 'تعديل',
    // 'user' => 'المشرفين',
    'user' => 'المستخدمين',
    'shipment' => 'الشحن',
    'business' => 'نوع العمل',
    'shipment_type' => 'نوع الشحنة',
    'receiver' => 'بيانات المشترى',
    'package' => 'بيانات المنتج ',
    'price' => 'سعر الشراء',
    'note' => 'ملاحظات',
    'add_note' => 'اضافة ملاحظة',
    'add' => 'اضف',
    'add_shipment' => 'اضافة الشحن',
    'status' => 'الحالة',
    'address' => 'العنوان',
    'export' => 'اكسل',
    'showshipment' => 'بيانات الشحن',
    'edit' => 'تعديل',
    'pickup' => 'شحن المنتجات ',
    'location' => 'العنوان',
    'time' => 'الوقت',
    'date' => 'التاريخ',
    'pickupid' => 'رقم الشحنة',
    'pickupdate' => 'تاريخ الشحنة',
    'add_pickup' => 'اضافة شحنة',
    'add_seller' => 'اضافة بائع',
    'package' => 'شحنات',
    'print' => 'طباعة',
    'details' => 'البيانات الشخصية',
    'admin' => 'الادمن',
    'permission' => 'الصلاحيات',
    'seller' => 'البائعين',
    'add_driver' => 'اضافة سائق',
    'driver' => 'سائق',
    // 'add_employee' => 'اضافة موظف',
    'type' => 'نوع',
    'cash' => 'المبلغ',
    'cost' => 'السعر النهائى',
    'comm' => 'عمولة السائق',
    'pdf' => 'pdf',
    'accountseller' => 'حسابات البائعين',
    'accountdriver' => 'حسابات السائقين',
    'accountsfile' => 'ملفات الحسابات',
    'shippername' => 'اسم البائع',
    'shipperphone' => 'هاتف البائع',
    'deliverycommission' => 'عمولة السائق',
    'scan' => 'ماسح',
    'designation' => 'تعيين',
    'optional' => 'اختيارى',
    'pickup_price' => 'سعر الاستلام',

    'seller_list' => 'قائمة البائعين',
    'add_seller' => ' اضافة بائع جديد',

    'driver_list' => 'قائمة السائقين',
    'add_driver' => 'اضافة سائق جديد',

    'employee_list' => 'قائمة الموظفين',
    'add_employee' => 'اضافة موظف جديد',
    'search_employee' => 'البحث عن الموظفين',
    'search' => 'البحث',
    'assign_pickup' => 'تعيين الشحنات',
    'allow_open' => 'مسموح الفتح',
    'onhold' => 'انتظار',
    'tracking' => 'تتبع المستخدم',
    'all-shippment' => 'كل الشحنات',
    'all-pickup' => 'كل الشحنات',
    'shipper-name' => 'اسم البائع',
    'name' => 'الاسم',
    'traching' => 'رقم تتبع',
    'add_address' => 'اضافة عنوان',
    'created_at' => 'تاريخ الانشاء',
    'created_by' => 'منشى من',
    'addfileshippment' => 'اضافة ملف الشحنات',
    'file_shipment' => 'ملف الشحنات',
    'guard' => 'المسجل',
    'assigned' => 'تعيين ',
    "permissions" => "الصلاحيات",
    "role" => "الصلاحية",
    "compound_info" => "معلومات المجمع",
    "upload_images" => "تحميل الصور",
    "browse" => "تصفح",
    "compounds" => "المجمعات",
    "add_new_compund" => "اضافة مجمع جديد",
    "edit_compund_info" => "تعديل بيانات المجمع",
    "compund_info" => "بيانات المجمع",
    "estates" => "العقارات",
    "building" => "العمارات",
    "apartments" => "الوحدات السكنية",
    "apartments" => "الوحدات السكنية",
    "one_apartment" => " شقة",
    "villa" => "فيلا",
    "a_land" => "ارض",
    "count" => "العدد",

    ////
    'add' => 'اضف',
    'create' => 'اضافه',
    'read' => 'عرض',
    'edit' => 'تعديل',
    'update' => 'تعديل',
    'delete' => 'حذف',
    'search' => 'بحث',
    'show' => 'عرض',
    'loading' => 'جاري التحميل',
    'print' => 'طبع',

    'confirm_delete' => 'تاكيد الحذف',
    'yes' => 'نعم',
    'no' => 'لا',

    'login' => 'تسجيل الدخول',
    'remember_me' => 'تذكرني',
    'password' => 'كلمه المرور',
    'password_confirmation' => 'تاكيد كلمه المرور',

    'added_successfully' => 'تم اضافه البيانات بنجاح',
    'updated_successfully' => 'تم تعديل البيانات بنجاح',
    'deleted_successfully' => 'تم حذف البيانات بنجاح',

    'no_data_found' => 'للاسف لا يوجد اي سجلات',
    'no_records' => 'للاسف لا يوجد اي سجلات',

    'clients' => 'العملاء',
    'client_name' => 'اسم العميل',
    'address' => 'العنوان',
    'previous_orders' => 'الطلبات السابقه',
    'orders' => 'الطلبات',
    'add_order' => 'اضف طلب',
    'edit_order' => 'تعديل طلب',

    // 'users' => 'المشرفين',
    'users' => 'المستخدمين',
    'first_name' => 'الاسم الاول',
    'last_name' => 'الاسم الاخير',
    'email' => 'البريد الاكتروني',
    'image' => 'صوره',
    'action' => 'اكشن',

    'permissions' => 'الصلاحيات',

    'categories' => 'الاقسام',
    'all_categories' => 'كل الاقسام',
    'name' => 'الاسم',
    'description' => 'الوصف',
    'products_count' => 'عدد الامنتجات',
    'related_products' => 'المنتجات المرتبطه',
    'category' => 'القسم',
    'show_products' => 'عرض المنتجات',
    // 'created_at' => 'تم اضافته',
    'actions' => 'اعدادات',
    'products' => 'المنتجات',
    'product' => 'المنتج',
    'quantity' => 'الكميه',
    'total' => 'المجموع',
    'purchase_price' => 'سعر الشراء',
    'price' => 'السعر',
    'sale_price' => 'سعر البيع',
    'stock' => 'المخزن',
    'profit_percent' => 'المكسب',
    'compound_update' => 'تم التعديل على المجمع',
    'compound_create' => ' تم اضافة مجمع جديد',
    'search_compounds' => 'البحث فى المجمعات',
    'add_compound' => 'اضافة مجمع',
    "compound_not_selected" => "لم يتم تحديد مجمع",
    "select_compound" => "اختر مجمع",



    "buildings" => "العمارات",
    "add_new_building" => "اضافة عمارة جديدة",
    "edit_building_info" => "تعديل بيانات العمارة",
    "building_info" => "بيانات العمارة",
    'building_update' => 'تم التعديل على العمارة',
    'building_create' => 'تم اضافة عمارة جديدة',
    'in_a_community' => 'في المجتمع',
    'standalone' => 'قائمة بذاتها',

    'add_building' => 'اضافة عمارة',
    'search_buildings' => 'البحث فى العمارات',
    "building_not_selected" => "لم يتم تحديد العمارة",
    "select_building" => "اختر عمارة",

    "apartment" => "الوحدات السكنية",
    "name_of_apartment" => "اسم او رقم الوحدة",
    "add_new_apartment" => "اضافة وحدة جديدة",
    "edit_apartment_info" => "تعديل بيانات الوحدة السكنية",
    "apartment_info" => "بيانات الوحدة السكنية",
    'apartment_update' => 'تم التعديل على الوحدة',
    'apartment_create' => 'تم اضافة الوحدة جديدة',
    'add_apartment' => 'اضافة الوحدة',
    'search_apartments' => 'البحث فى الوحدات السكنية',
    'space' => 'مساحة الوحدة',
    'floor' => 'رقم الطابق',
    'bedroom' => 'عدد غرف النوم',

    'bathroom' => 'عددالحمامات ',
    'councils' => 'عدد المجالس',
    'lounges' => 'عدد الصالات',
    'furnishing_condition' => 'حالة التأثيث',
    'electricity_meter_number' => 'رقم عداد الكهرباء',
    'water_meter_number' => 'رقم عداد المياه',
    'parking' => 'موقف السيارات',
    'type_of_kitchen' => 'نوع المطبخ',
    'type_of_air_conditioning' => 'نوﻉ التكييف',
    'window' => 'شباك',
    'split' => 'سبليت',
    'central' => 'مركزى',
    'not_available' => 'لا يتوفر',

    'open' => 'مفتوح',
    'closed' => 'مغلق',

    'submit' => 'إرسال',
    'edit' => 'تعديل',
    'cancel' => 'إلغاء',
    'excel_file' => 'ملف الاكسل',
    'add_excel_file' => 'اضف ملف الاكسل',
    'tenants' => 'مستاجرين',
    "tenant_info" => "بيانات المستاجرين",
    'search_tenants' => 'البحث فى المستاجرين',
    'add_tenant' => 'اضافة مستاجر',
    "add_new_tenant" => "اضافة مستاجر جديد",
    "one_tenant" => " مستاجر ",

    "rental_details" => "تفاصيل الإيجار",
    "contract_number" => "رقم العقد",
    "contract_starting_date" => "تاريخ بداية العقد",
    "contract_end_date" => "تاريخ نهاية العقد",
    "total_rental_amount" => "إجمالي مبلغ الإيجار",
    "guarantee_amount" => "مبلغ الضمان",
    "number_of_batches" => "عدد الدفعات",
    "select" => "اختر",
    "nationality" => "الجنسية",
    "id_number" => "رقم الهوية",
    "from" => "تاريخ البداية",
    "to" => "تاريخ النهاية",
    "total_amount_of_rent" => "اجمالى مبلغ الايجار",
    "guarantee_amount" => "مبلغ الضمان",
    "payment_method" => "طريقة الدفع",
    "payment_at_the_beginning_of_the_contract" => "الدفع في بداية العقد",
    "pay_with_rent_payments" => "الدفع مع دفعات الإيجار",
    "tenant_data" => "بيانات المستأجر",
    "lease_contract_data" => "بيانات عقد الإيجار",
    "or_add_new_tenant" => "أو قم بإضافة مستأجر جديد",
    "finance" => "المالية",
    "evacuation" => "إخلاء",
    "renewal" => "تجديد",
    "all_operations" => "جميع العمليات",
    "add_date" => "اضف التاريخ",
    "my_profile" => "حسابى",
    "contracts" => "عقود",
    "owners" => "الملاك",
    "owner_info" => "بيانات الملاك",
    "no_contracts" => "لا يوجد عقود",
    "no_rating" => "لا يوجد تقييم للمستاجر",

    "payment" => "دفع الإيجار الشهري",
    "cleanliness" => "النظافة",
    "attention_at_home" => "الاهتمام في المنزل",
    "annoyance" => "الازعاج",

    "commercial_station" => "محطة تجارية",
    "storehouse" => "مستودع",
    "office" => "مكتب",
    "showroom" => "معرض",
    "residential" => "سكنى",
    "commercial" => "تجارى",

    "arabic" => "العربية",
    "english" => "الانجليزية",
    "id_photo" => "صورة الهوية",
    "information" => "معلومات",
    "active" => "فعال",
    "non_active" => "غير فعال",
    "view" => "عرض",
    "customer" => "عميل",
    "customers" => "العملاء",
    "add_customer" => "اضافة عميل جديد",
    "search_customer" => "البحث عن العملاء",
    "all_customer" => " كل العملاء",

    "all_branch" => " كل الفروع",
    "add_branch" => "اضافة فرع جديد",
    "search_branch" => "البحث فى الفروع",
    "branch" => "فرع",

    "all_product" => "كل المنتجات",
    "add_product" => "اضافة منتج جديد",
    "search_product" => "البحث فى المنتجات",
    "product" => "المنتجات",

    "serial_number" => "رقم سري",

    "all_installment" => "كل الاقساط",
    "add_installment" => "اضافة قسط جديد",
    "search_installment" => "بحث فى القسط",
    "installment" => "القسط",
    "formule" => "الصيغة",

    "installment_method" => "طرق التقسيط",
    "installment_months" => "اشهر التقسيط",
    "installment_amount" => "مبلغ القسط",
    "presenter" => "المقدم",
    "guarantor" => "الكفيل",
    "without_guarantor" => "بدون الكفيل",

    "all_order" => "كل الطلبات",
    "orders" => "الطلبات",
    "add_order" => "اضافة طلب",
    "orderId" => "رقم الطلب",
    "invoiceId" => "رقم الفاتورة",
    "search_order" => "البحث فى الطلبات",
    "order" => "الطلبات",
    "order_details" => "تفاصيل الطلب",

    "installment_date" => "موعد القسط",

    "contract" => "العقد",
    "all_contract" => "كل العقود",
    "search_contract" => "بحث فى العقود",

    "installments" => "الاقساط",
    "total_installments" => "عدد الاقساط",
    "pay_date" => "موعد الدفع",
    "paid_date" => "تم الدفع",
    "installment_number" => "رقم القسط",
    "invoice" => "فاتورة",

    "sign_in" => "تسجيل دخول",
    "forgot_password" => "نسيت كلمة المرور",
    "enter_your_phone" => "أدخل هاتفك لإعادة تعيين كلمة المرور الخاصة بك.",
    "collector" => "المحصل",

    "paid" => "دفع",
    "download-invoice" => "تحميل الفاتورة",
    "setting" => "اعدادات",
    "setting_acount" => "اعدادات الحساب",
    "profile_details" => "تفاصيل الحساب",
    "edit_profile" => "تعديل الحساب",

    "new_password" => "كلمة سر جديدة",
    "confirm_new_password" => "تاكيد كلمة سر جديدة",

    "paidontime" => "دفعت فى الموعد",
    "paidlate" => "دفعت متاخرة",
    "paidearly" => "دفعت باكرا",

    "paidup" => "المدفوع",
    "unpaid" => "غير مدفوع",

    "invoices" => "الفواتير",
    "overview" => "نظرة عامة",
    "activity" => "نشاط",

    "collected_installments" => "الأقساط المحصلة",

    "installments_payable" => "أقساط مستحقة الدفع",

    "others" => "اخرى",

    "Paid_installments" => "الاقساط المدفوعة",

    "products_ordered" => "المنتجات المطلوبة",

    "select_product" => "حدد المنتج",
    "add_products_to_this_order" => "أضف منتجات لهذا الطلب",
    "select_one_or_more_product_from_the_list_below_by_ticking_the_checkbox" => "حدد منتجًا واحدًا أو أكثر من القائمة أدناه عن طريق تحديد خانة الاختيار.",


    "total_value_contract" => "القيمة الإجمالية للعقود",

    "total_value_contract" => "القيمة الإجمالية للعقود",

    "total_order_approved" => "العدد الإجمالي للطلبات المعتمدة",

    "monthy_installment" => "الاقساط الشهرية",
    "all_monthy_installment" => "كل الاقساط الشهرية",
    "waiting" => "انتظار",
    "late" => "متاخر",

    "already_a_member" => "انت مسجل فعلا ?",

    "order_summary" => "ملخص الطلب",
    "guarantor_details" => "تفاصيل الكفيل",
    "customer_details" => "تفاصيل العميل",
    "order_details" => "تفاصيل الطلب",
    "print_contract" => "طباعة العقد",
    "add_new_contract" => "اضافة عقد جديد",
    "date_added" => "تاريخ الاضافة",
    "payment_method" => "طريقة التقسيط",
    "total" => "المجموع",
    "grand_total" => "المجموع الإجمالي",

    "branch_name_en" => "اسم الفرع بالانجليزى",
    "branch_name_ar" => "اسم الفرع بالعربى",

    "company_name" => "اسم الشركة",

    "company" => "الشركة",
    "is_company" => "شركة",
    "is_not_company" => "ليست شركة",

    "add_check" => "اضافة شيكات",
    "print_check" => "طباعة شيكات",

    "apps" => "التطبيقات",
    "hr" => "الموارد البشرية",
    "select_option" => "حدد اختيار",

    "lawyer_fees" => "اتعاب المحامى",

    "create_boarding_successfully" => "تم انشاء عنصر جديد",
    "update_boarding_successfully" => "تم تعديل العنصر بنجاح",
    "delete_boarding_successfully" => "تم حذف العنصر بنجاح",


    "create_successfully" => "تم انشاء عنصر جديد",
    "update_successfully" => "تم تعديل العنصر بنجاح",
    "delete_successfully" => "تم حذف العنصر بنجاح",

    "done_successfully" => "تم العملية بنجاح",

    "all_boarding" => "كل الواجهات",
    "boardings" => "الواجهات",
    "boarding" => "الواجهة",
    "add_new_boarding" => "اضافة واجهة جديدة",

    "place" => "المكان",

    "place_boarding" => "مكان الواجهة",

    "title" => "العنوان",

    "are_you_sure" => "هل انت متاكد ؟",
    "you_won_be_able_to_revert_this" => "! لن تكون قادرا على التراجع عن هذا",
    "yes_delete_it" => "نعم , احذفه",

    "privacy" => "الخصوصية",

    "duration-agreement" => "المدة الاتفاق",

    "condition" => "الشروط والأحكام",

    "contact" => "تواصل معنا",

    "f_name" => "الاسم الاول",
    "l_name" => "الاسم الاخير",
    "subject" => "الموضوع",
    "message" => "الرسالة",

    "avatar" => "صورة",

    "ads" => "الاعلانات",
    "add_new_ads" => "اضافة اعلان جديد",
    "is_home" => "عرض فى الرئيسية",

    "countries" => "الدول",
    "country" => "الدولة",
    "add_new_country" => "اضافة دولة جديدة",

    "cities" => "المدن",
    "cities" => "المدن",

    "failed_to_save" => "فشل فى الحفظ",
    "saved_successfully" => "تم الحفظ بنجاح",

    "roomtypes" => "انواع الغرف",
    "roomtype" => "نوع الغرفة",
    "add_new_roomtype" => "اضافة نوع غرفة جديدة",

    "rooms" => "الغرف",
    "is_favorite" => "المميزة",
    "favorite" => "مميز",

    "groups" => "مجموعات",

    "gla_event" => "احداث غلا",
    "add_new_event" => "اضافة حدث جديد",


    "gla_team" => "فريق غلا",
    "add_new_team" => "اضافة فريق جديد",

    "agencies" => "وكالات",
    "agency" => "وكالة",
    "add_new_agency" => "اضافة وكالة جديدة",

    "user_id" => "رقم المستلم",
    "sender" => "المرسل",
    "recipient" => "المستلم",
    "wallet" => "المحفظة",

    "dollar" => "دولار",

    "followers" => "المتابعون",
    "following" => "يتابعهم",

    "articles" => "مقالات",
    "post" => "منشور",
    "posts" => "منشورات",
    "add_new_post" => "اضافة منشور جديد",

    "pages" => "صفحات",

];
