<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_customers_table">
    ...
    <tbody class="fw-semibold text-gray-600" id="table_data">
        ...
        <tr>
            <!--begin::Name=-->
            <td>
                <a href="{{route('user.show',$user->id)}}" class="text-gray-800 text-hover-primary mb-1">{{
                    $user->name }}</a>
            </td>
            <!--end::Name=-->
            ...
            <td>
                <div class="checkbox-wrapper-10">
                    ...
                    id="cb_{{$user->id}}"
                    class="tgl tgl-flip">
                    <label onclick="changeStatus('{{$user->id}}')" for="cb_{{$user->id}}"
    ...
    <div class="row">
        <div class="col-sm">
            <div class="pagination">
                ...
                @else
                <li class="page-item"><a class="page-link" onclick="getUser('{{ $users->currentPage() - 1 }}')">‹</a>
                </li>
                @endif
                @php
                use App\Helpers\Helper;

                $pagination = Helper::getPagination($users->currentPage(), $users->lastPage());
                @endphp
                @php
                ...
                @if ($startPage > 1)
                <li class="page-item"><a class="page-link" onclick="getUser(1)">1</a></li>
                @if ($startPage > 2)
                <li class="page-item disabled"><span class="page-link">...</span></li>
                @endif
                @endif
                ...
                @for ($i = $startPage; $i <= $endPage; $i++) <li
                    class="page-item {{ $i == $users->currentPage() ? 'active' : '' }}">
                    <a class="page-link" onclick="getUser('{{ $i }}')">{{ $i }}</a>
                    </li>
                    @endfor
                    ...
                    @if ($endPage < $users->lastPage())
                        @if ($endPage < $users->lastPage() - 1)
                            <li class="page-item disabled"><span class="page-link">...</span></li>
                            @endif
                            <li class="page-item"><a class="page-link" onclick="getUser('{{ $users->lastPage() }}')">{{
                                    $users->lastPage() }}</a></li>
                            @endif
                            ...
                            <li class="page-item"><a class="page-link"
                                    onclick="getUser('{{ $users->currentPage() + 1 }}')">›</a></li>
                            @else
                            <li class="page-item disabled"><span class="page-link">›</span></li>
                            @endif
            </div>
